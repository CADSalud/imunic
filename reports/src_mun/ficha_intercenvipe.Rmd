...
# municipios_selec <- c("Cuajimalpa de Morelos", 9)
# municipios_selec <- c("Sultepec", 15)
# municipios_selec <- c("Texcaltitlán", 15)
# municipios_selec <- c("Jilotepec", 30)
municipios_selec <- c("Chumatlán", 30)
...
La diferencia entre el ingreso por persona del 25% más bajo y el 75% más alto es de `r format_fun(round(tab_interc$ingreso_q75 - tab_interc$ingreso_q25))`. `r frase`
...
row_percepcion_fun <- function(row){
  frase <- paste0("El ", row$valor, "% ", row$Descripción, "." )
  frase
}
tab <- tab_envipe %>% 
...
if(sum(is.na(tab$valor)) < nrow(tab)){
  ...
  lista_in <- tab_in %>% 
    ...
    .$res
  cat("En esta sección se presentan las problemáticas que más de la mayoría ha visto al rededor de su vivienda:\n\n")  
...
\subsubsection{4.3 Ingreso trimetral del hogar promedio.} 

(DAVID: src/indicadores_enigh.R   enigh2016mun::ingr_prom)
...
(DAVID: src/indicadores_enigh.R   enigh2016mun::phog_arriba_lbm)
...
(DAVID: damus/bd_imunac_login::dt_empleo_uso_suelo)